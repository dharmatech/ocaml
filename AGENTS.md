...
- Before running or diagnosing any Plan 9 toolchain setup or build task, read
  the top-level `README.md` completely from the current authoritative checkout.
  Treat it as the canonical end-to-end entry point.
- Before configuring, building, testing, installing, or diagnosing OCaml
  itself, also read `build-aux/plan9/README.md` completely for the specific
  helper procedure. Follow the documented shell setup exactly. Enter APE
  through `ape/psh`; do not approximate that environment with manual namespace
  binds or ad hoc command adapters. If a documented command cannot be used,
  stop and identify the exact discrepancy before substituting anything.
